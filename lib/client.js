        ...
        switch(message.command) {
            ...
            case "USERNOTICE":
                ...
                var numbOfSubs = message.tags["msg-param-mass-gift-count"];
								var showStreak = message.tags["msg-param-should-share-streak"] || 0;
								var streakMonths = _.get(~~message.tags["msg-param-streak-months"], null);
                userstate["message-type"] = msgid;

                if (msgid === "resub") {
                    this.emits(["resub", "subanniversary"], [
                        [channel, username, months, msg, userstate, {prime, plan, planName}, showStreak, streakMonths]
                    ]);
                }

                // Handle sub
                else if (msgid == "sub") {
                    this.emit("subscription", channel, username, {prime, plan, planName}, msg, userstate, showStreak, streakMonths);
                }
